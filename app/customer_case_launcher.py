...
if PYSIDE6_AVAILABLE:
    class CustomerCaseMainWindow(QMainWindow):
        ...
        def _build_ui(self) -> None:
            ...
            subtitle = QLabel("读取已有的 ModeB 输出报告，并复用 CapacityOptimizer 的共享工作目录生成产品分析 Excel。")
            ...
            self.use_latest_checkbox = QCheckBox("自动调用共享 output 中最新的 ModeB 报告")
        ...
        def _refresh_latest_report_hint(self) -> None:
            ...
            try:
                ...
                self.latest_report_hint.setText("共享 output 目录下还没有可用的 ModeB 报告。")
                return
