...
def _summary_row(
    *,
    ...
    return {
        ...
        "Capacity_Internal_Tons": round(sum(_to_float(row.get("Filled_Tons")) for row in capacity_rows), 4),
        "Routing_Internal_Product_Count": len(routing_internal_rows),
        "Routing_Internal_Tons": round(sum(_to_float(row.get("Filled_Tons")) for row in routing_internal_rows), 4),
        "Routing_Outsourced_Product_Count": len(routing_outsourced_rows),
        "Routing_Outsourced_Tons": round(sum(_to_float(row.get("Filled_Tons")) for row in routing_outsourced_rows), 4),
        "Unmet_Product_Count": len(unmet_rows),
        "Unmet_Tons": round(sum(_to_float(row.get("Attributed_Unmet_Tons")) for row in unmet_rows), 4),
    }
